...
include('config.php');
$ID = $_GET['id'];
$up = mysqli_query($con, "SELECT * FROM category where id=$ID");
...
if(isset($_POST['insert_cat'])){
    ...
    $insert_cat = "update category set name='$cat_title'";
    
    $run_cat = mysqli_query($con, $insert_cat);
    
    if($run_cat){
    
    echo "<script> alert(' Categorie est modifiée')</script>";
                        ...
                        <div class="case">
                            <div class="social-boxes pl-24 pt-10 pb-10 bg-white rad-10">
                                ...
                                <div class="mb-15">
                                    ...
                                    <label class="fs-14 c-pri display-block margTop-10 margBottom-5" for="first">modifier le nom de la catégorie:</label>
                                    <br>
                                    <input class="b-none border-ccc p-10 rad-6 d-block w-95 " type="text" id="first" name="category_name" value='<?php echo $data['name']?>'/>
                                    <br>               
                                    <input class="btn" type="submit" name="insert_cat" value="modifier la categorie" />
                                    </form>
                                </div>
                            </div>                                                            
                        </div>
